            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                        ...
                        <td class="border px-4 py-2">{{product?.text}}</td>
                        ...
                        <td class="px-6 py-4">
                            <button @click="modalEdit = true, editLocationsValues(product)" class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
                              Edit
                            </button>
                            &nbsp;
          ...
          <div class="relative p-4 w-full max-w-md max-h-full">
              <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <div class="p-4 md:p-5 text-center">
                      ...
                      <button @click="deleteLocations()" data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                          Yes, I'm sure
                      </button>
  ...
  <div v-if="modalEdit"  tabindex="-1" aria-hidden="true" class="a overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative p-4 w-full max-w-md max-h-full" >
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" >
              ...
              <form @submit.prevent="editLocations" class="p-4 md:p-5">
                  <div class="grid gap-4 mb-4 grid-cols-2">
                      <div class="col-span-2">
                          <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name *</label>
                          <input v-model="data.name" @input="inputName($event.target.value)" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" required="">
                      </div>
                      <div class="col-span-2">
                          <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Text *</label>
                          <input v-model="data.text" @input="inputText($event.target.value)" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" required="">
                      </div>
                      <div class="col-span-2">
                          <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Picture *</label>
...
import "vue3-toastify/dist/index.css";
  ...
  const inputName = (product) =>{
      editName.value = product
  }
  const inputText = (product) =>{
      editText.value = product
  }
  ...
  const onChange = () =>{
      ...
      data.value.text = editText,
      data.value.images = editpicFile
  }
  const editLocationsValues = (product) =>{
      productId.value = product.id,
      editName.value = product.name,
      editText.value = product.text,
  ...
  const deleteLocations = async () =>{
      try{
          const response = await fetch(`https://autoapi.dezinfeksiyatashkent.uz/api/locations/${productId.value}`,{
              method: 'DELETE',
              headers: {
                  'Authorization': `Bearer ${accessToken.value}`,
  ...
  const editLocations = async () => {
      ...
      try {
          const res = await fetch(`https://autoapi.dezinfeksiyatashkent.uz/api/locations/${productId.value}`, {
              method: 'PUT',
              body: formData,
              headers: {
                  'Authorization': `Bearer ${accessToken.value}`,
