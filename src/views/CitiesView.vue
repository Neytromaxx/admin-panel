          ...
          <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                  <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                      ...
                      <td class="px-6 py-4">
                          <button @click="modalEdit = true, editCitiesValues(product)" class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
                            Edit
                          </button>
                          &nbsp;
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 text-center">
                    ...
                    <button @click="deleteCities()" data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                        Yes, I'm sure
                    </button>
...
<div v-if="modalEdit"  tabindex="-1" aria-hidden="true" class="a overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full" >
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" >
            ...
            <form @submit.prevent="editCities" class="p-4 md:p-5">
                <div class="grid gap-4 mb-4 grid-cols-2">
                    <div class="col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name *</label>
...
const editCitiesValues = (product) =>{
...
const deleteCities = async () =>{
    try{
        const response = await fetch(`https://autoapi.dezinfeksiyatashkent.uz/api/cities/${productId.value}`,{
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${accessToken.value}`,
...
const editCities = async () => {
    ...
    try {
        const res = await fetch(`https://autoapi.dezinfeksiyatashkent.uz/api/cities/${productId.value}`, {
            method: 'PUT',
            body: formData,
            headers: {
                'Authorization': `Bearer ${accessToken.value}`,
